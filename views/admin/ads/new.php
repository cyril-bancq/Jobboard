...
if (!empty($_POST)) {
    ...
    if ($v->validate()) {
        $adsTable->create($ads);
        header('Location: ' . $router->url('admin_ads_edit', ['id' => $ads->getID()]) . '?created=1');
        exit();
    } else {
        $errors = $v->errors();
    }
...
<body class="d-flex flex-column h-100">
    ...
    <?php if (!empty($errors)): ?>
        <div class="text-center">
            <div class="alert alert-danger">This advertissement has not been created, please correct your mistakes</div>
        </div>
    <?php endif ?>
    <h3>Create a advertissement</h3>
